...
export default function CreateStoreItem() {
  ...
  async function handleSubmit(e) {
    ...
    else setError((await res.json()).error || "Failed to create item");
  }
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow space-y-4 max-w-xl">
        ...
        <input name="link" placeholder="Link to resource" required className="w-full border p-3 rounded" />
        <div>
          <label className="block text-sm mb-1">Image</label>
          <input name="img" type="file" accept="image/*" required className="w-full" />
        </div>
        <button className="bg-primary text-white px-6 py-3 rounded">Save Item</button>
      </form>
    </div>
  );
}
